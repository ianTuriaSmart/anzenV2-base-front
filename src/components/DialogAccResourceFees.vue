      ...
      <div v-if="!isLoading" style="max-height: 60vh; overflow-y: auto;">
        <q-list separator class="q-mb-md">
          <q-item v-for="tariff in tariffs" :key="tariff.idTariff">
            <q-item-section>
              <q-item-label>• {{ tariff.tariff }} {{ tariff.amount }} €</q-item-label>
            </q-item-section>
          </q-item>
        </q-list>
...
import {  apiGetAccessTariffs } from 'src/services/apiAccess';
...
onMounted(async () => {
  ...
  const response = await apiGetAccessTariffs(props.selectedResource.idResource);
